...
def notebook_path(verbose=False):
  ...
  if verbose: print(connection_file)
  ...
  if verbose: print('---begin iterating through servers---')
  
  for srv in oldstyle_servers + newstyle_servers:
      nbkdir = srv.get('notebook_dir', srv.get('root_dir'))
      try:
          if srv['token']=='' and not srv['password']:  # No token and no password, but token may be present in env
              ...
              req = urllib.request.urlopen(
                  srv['url']+f'api/sessions?token={envtoken}',
                  timeout=2
              ...
              req = urllib.request.urlopen(srv['url']+'api/sessions?token='+srv['token'], timeout=2)
          
          sessions = json.load(req)
          if verbose: print(f'sessions:{sessions}')
              
          for sess in sessions:
              if verbose: print(f"kernelid:{sess['kernel']['id']}")
      ...
      except (urllib.error.URLError, TimeoutError, timeout) as inst:  # think we have the relevant ones! # 2021-07-23 added timeout from socket
          if verbose:
...
def fileloc(input_globals, depth=2, verbose=False):
  ...
  if '__file__' in input_globals:
      if verbose: print('__file__ in globals, use that')
      pyfile =  input_globals['__file__']
  else:
      try:
          pyfile = notebook_path(verbose=verbose)
      except (RuntimeError, MultipleInstanceError):  # multiple instance if running ipython shell
          pyfile =  'manual_run.'
...
def setup_logging(loggername, filename=None, add_ts=False):
  ...
  logger_format = logging.Formatter('[%(asctime)s] %(name)s - %(levelname)s - %(message)s', datefmt='%Y-%m-%d %H:%M:%S')
  ...
  if add_ts:
      ...
      if len(name_parts) == 1: name_parts += ['txt']
